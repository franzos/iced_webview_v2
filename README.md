...
> **Heads up — the [`next`](https://github.com/franzos/iced_webview_v2/tree/next) branch tracks iced `master` and blitz `0.3.0-beta.2` (both on wgpu 29).** Blitz there rasterizes directly on iced's `wgpu::Device` via a shared Vello renderer - no CPU pixel readback. Servo `0.5` and blitz beta-2 share stylo 0.20, so all four engines build together on that branch. It can't be published to crates.io or merged here until iced ships a release on wgpu 29, so `main` stays on the released iced 0.14.
...
- **Banded rendering, no partial repaint** — the engine rasterizes a band of two viewport heights around the scroll position. Scrolling inside the band only moves the texture offset on the GPU; leaving it, or any style, hover, or resource change, re-rasterizes the whole band. Blitz is pre-alpha and doesn't yet support dirty-rect repaint like Firefox/Chrome. Blitz still owns the scroll position (`viewport_scroll`), so the texture stays bounded by window size regardless of document length.
- **GPU rasterization with CPU readback** — the viewport is rasterized on the GPU via `anyrender_vello`, then read back to a CPU buffer and re-uploaded to iced's wgpu device for display. The redundant roundtrip is unavoidable until iced and `anyrender_vello` agree on a wgpu major version (currently 27 vs 28). See [TODO.md](TODO.md). The `blitz-vello-cpu` feature swaps in `anyrender_vello_cpu` instead: no second wgpu device and no GPU readback, at the cost of software rasterization. Useful with iced's tiny-skia renderer or on machines without a usable GPU.
